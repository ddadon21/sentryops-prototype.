              ...
              <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-4">
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    <h2 className="text-2xl lg:text-3xl font-bold text-white">Subject Records</h2>
                ...
                {selectedTab === 'employment' && (
                  <div className="space-y-4">
                    {subjectData.employment.map((job, idx) => (
                      <div key={idx} className={`bg-slate-900/40 rounded-xl p-4 ${job.isCurrentLEO ? 'border border-blue-500/30' : ''}`}>
                        {/* LEO Status Banner */}
                        {job.isCurrentLEO && (
                ...
                {selectedTab === 'certifications' && (
                  <div className="space-y-4">
                    ...
                    {subjectData.certifications.map((cert, idx) => (
                      <div key={idx} className={`bg-slate-900/40 rounded-xl p-4 ${cert.status === 'critical' ? 'border-2 border-red-500/50' : cert.status === 'expiring_soon' ? 'border border-amber-500/30' : ''}`}>
                        {/* Critical Badge */}
                ...
                {selectedTab === 'references' && (
                  <div className="space-y-4">
                    ...
                    {subjectData.references.map((ref, idx) => (
                      <div key={idx} className={`bg-slate-900/40 rounded-xl p-4 ${ref.isIAReview ? 'border-2 border-amber-500/50' : ''}`}>
      ...
      {resourcesOpen && (
        <div className="fixed bottom-24 right-6 w-full max-w-md bg-slate-900/95 backdrop-blur-xl border border-slate-700/50 rounded-2xl shadow-2xl flex flex-col z-40 mx-4 sm:mx-0 max-h-[600px]">
          <div className="p-4 border-b border-slate-700/50">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">Documentation and assistance</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-4">
            ...
            <div>
              ...
              <div className="space-y-2">
                <button className="w-full flex items-center gap-3 p-3 bg-slate-800/40 hover:bg-slate-800/60 rounded-xl text-left transition-colors">
                  ...
                  <div>
                    <p className="text-sm text-white font-medium">Export Subject Records</p>
                    <p className="text-xs text-slate-500">Download complete file as PDF</p>
                  </div>
                </button>
                <button className="w-full flex items-center gap-3 p-3 bg-slate-800/40 hover:bg-slate-800/60 rounded-xl text-left transition-colors">
                  <FileText className="w-4 h-4 text-blue-400" />
                  <div>
                    <p className="text-sm text-white font-medium">Generate Verification Report</p>
                    <p className="text-xs text-slate-500">Create summary of all verifications</p>
                  </div>
                </button>
                <button className="w-full flex items-center gap-3 p-3 bg-slate-800/40 hover:bg-slate-800/60 rounded-xl text-left transition-colors">
                  <Mail className="w-4 h-4 text-green-400" />
                  <div>
                    <p className="text-sm text-white font-medium">Request IA Records Follow-up</p>
                    <p className="text-xs text-slate-500">Send reminder to Metro Atlanta PD</p>
                  </div>
                </button>
              </div>
            </div>
            ...
            {/* Documentation */}
            <div>
              <h4 className="text-xs font-semibold text-slate-400 uppercase mb-3">Documentation</h4>
              <div className="space-y-2">
                <a href="#" className="flex items-center gap-3 p-3 bg-slate-800/40 hover:bg-slate-800/60 rounded-xl transition-colors">
                  <BookOpen className="w-4 h-4 text-slate-400" />
                  <span className="text-sm text-slate-300">Lateral Transfer Requirements</span>
                  <ExternalLink className="w-3 h-3 text-slate-500 ml-auto" />
                </a>
                <a href="#" className="flex items-center gap-3 p-3 bg-slate-800/40 hover:bg-slate-800/60 rounded-xl transition-colors">
                  <BookOpen className="w-4 h-4 text-slate-400" />
                  <span className="text-sm text-slate-300">Brady/Giglio Disclosure Guide</span>
                  <ExternalLink className="w-3 h-3 text-slate-500 ml-auto" />
                </a>
                <a href="#" className="flex items-center gap-3 p-3 bg-slate-800/40 hover:bg-slate-800/60 rounded-xl transition-colors">
                  <BookOpen className="w-4 h-4 text-slate-400" />
                  <span className="text-sm text-slate-300">IA Records Request Procedures</span>
                  <ExternalLink className="w-3 h-3 text-slate-500 ml-auto" />
                </a>
                <a href="#" className="flex items-center gap-3 p-3 bg-slate-800/40 hover:bg-slate-800/60 rounded-xl transition-colors">
                  <BookOpen className="w-4 h-4 text-slate-400" />
                  <span className="text-sm text-slate-300">Certification Renewal Requirements</span>
                  <ExternalLink className="w-3 h-3 text-slate-500 ml-auto" />
                </a>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="bg-slate-800/40 rounded-xl p-4">
                <p className="text-sm text-slate-300 mb-3">
                  Contact the Background Investigations Unit supervisor for questions about this case or verification procedures.
                </p>
                <div className="space-y-2 text-sm">
                  <div className="flex items-center gap-2 text-slate-400">
                    <Phone className="w-4 h-4" />
                    <span>(770) 513-5100 ext. 4250</span>
                  </div>
                  <div className="flex items-center gap-2 text-slate-400">
                    <Mail className="w-4 h-4" />
                    <span>bi.supervisor@gcso.gov</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="p-4 border-t border-slate-700/50">
            <p className="text-xs text-slate-500 text-center">
              Case {caseInfo.caseId} • Last updated {caseInfo.lastUpdated}
            </p>
          </div>
        </div>
      )}
