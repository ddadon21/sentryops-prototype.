        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-slate-900 border border-slate-700/50 rounded-2xl p-6 max-w-3xl w-full shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-start justify-between mb-6">
              <div>
                <h3 className="text-xl font-bold text-white mb-1">{selectedReport.name}</h3>
                <p className="text-sm text-slate-400">{selectedReport.type} Report • {selectedReport.period}</p>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-4">
                ...
                <p className="text-sm text-white">{selectedReport.generated}</p>
              </div>
              <div className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-4">
                <p className="text-xs text-slate-500 mb-1">Generated By</p>
                <p className="text-sm text-white">{selectedReport.generatedBy}</p>
              </div>
              <div className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-4">
                <p className="text-xs text-slate-500 mb-1">File Size</p>
                <p className="text-sm text-white">{selectedReport.fileSize}</p>
              </div>
              <div className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-4">
                <p className="text-xs text-slate-500 mb-1">Pages</p>
                <p className="text-sm text-white">{selectedReport.pages} pages</p>
              </div>
            </div>

            <div className="mb-6">
              <h4 className="text-sm font-semibold text-white mb-3">Report Summary</h4>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <button className="px-4 py-2.5 bg-slate-800/40 hover:bg-slate-800/60 border border-slate-700/50 rounded-xl text-white font-medium transition-all flex items-center justify-center gap-2 text-sm">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="flex-1 overflow-y-auto p-4 lg:p-6">
          <div className="max-w-7xl mx-auto">
            ...
            {/* Page Title */}
            <div className="mb-6">
              <h2 className="text-2xl lg:text-3xl font-bold text-white mb-1">HR Reports & Personnel Analytics</h2>
            ...
            <div className="mb-6 bg-slate-800/40 border border-slate-700/50 rounded-xl p-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs text-slate-400">
                <div className="flex items-center gap-2">
                  ...
                  <span>HR Director: <span className="text-slate-300">Personnel Admin</span> | Sheriff: <span className="text-slate-300">Keybo Taylor</span> | Report Admin: <span className="text-slate-300">HR Specialist J. Martinez</span></span>
                </div>
                <div className="flex items-center gap-2">
                  <FileText className="w-3.5 h-3.5 text-slate-500" />
                  <span>Total Personnel: <span className="text-slate-300">178 (164 sworn, 14 civilian)</span> | Last Report: <span className="text-slate-300">Q4 Hiring Summary (Dec 3, 2024)</span></span>
                </div>
              </div>
            ...
            <div className="mb-6 space-y-2">
              <div className="flex items-start gap-3 p-3 bg-amber-500/10 border border-amber-500/20 rounded-lg">
                ...
                <p className="text-sm text-slate-300">Hiring velocity: Time-to-fill <span className="text-amber-400 font-medium">127 days</span> (Deputy Sheriff) — background investigation bottleneck identified</p>
              </div>
              <div className="flex items-start gap-3 p-3 bg-amber-500/10 border border-amber-500/20 rounded-lg">
                <AlertTriangle className="w-4 h-4 text-amber-400 mt-0.5 flex-shrink-0" />
                <p className="text-sm text-slate-300">Training compliance at <span className="text-amber-400 font-medium">98.8%</span> (POST): 2 deputies with expired firearms, 4 certifications expiring next 30 days</p>
              </div>
              <div className="flex items-start gap-3 p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg">
                <Info className="w-4 h-4 text-blue-400 mt-0.5 flex-shrink-0" />
                <p className="text-sm text-slate-300">PTO average balance: <span className="text-blue-400 font-medium">72 hours</span> — 8 employees at accrual cap (risk of losing leave)</p>
              </div>
              <div className="flex items-start gap-3 p-3 bg-amber-500/10 border border-amber-500/20 rounded-lg">
                <AlertTriangle className="w-4 h-4 text-amber-400 mt-0.5 flex-shrink-0" />
                <p className="text-sm text-slate-300">Performance reviews: <span className="text-amber-400 font-medium">89% completion</span> — 3 reviews pending supervisor approval, 1 severely overdue (510+ days)</p>
              </div>
            </div>
            ...
            {(activeSection === 'all' || activeSection === 'command') && (
              <div className="mb-8">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    <h3 className="text-lg font-semibold text-white">Sheriff's Command Briefing Reports</h3>
                    <p className="text-xs text-slate-400">Reports for Sheriff Keybo Taylor and command staff decision-making</p>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  {commandReports.map(report => {
                    ...
                    return (
                      <div key={report.id} className="bg-slate-800/40 border border-slate-700/50 rounded-xl overflow-hidden">
                        <button
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500">
                              <span>Frequency: <span className="text-slate-300">{report.frequency}</span></span>
                              <span>Next Due: <span className={report.nextDueUrgent ? 'text-red-400 font-medium' : 'text-slate-300'}>{report.nextDue}</span></span>
                            </div>
                          </div>
                          {isExpanded ? <ChevronUp className="w-5 h-5 text-slate-400 mt-1" /> : <ChevronDown className="w-5 h-5 text-slate-400 mt-1" />}
                        </button>
                          ...
                          <div className="px-5 pb-5 space-y-4">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                              <div className="bg-slate-900/40 border border-slate-700/30 rounded-xl p-3">
                                ...
                                <p className="text-sm text-slate-300">{report.recipients}</p>
                              </div>
                              <div className="bg-slate-900/40 border border-slate-700/30 rounded-xl p-3">
                                <p className="text-xs text-slate-500 mb-1">Last Generated</p>
                                <p className="text-sm text-slate-300">{report.lastGenerated}</p>
                              </div>
                            </div>

                            <div className="bg-slate-900/40 border border-slate-700/30 rounded-xl p-4">
                              <h5 className="text-sm font-semibold text-white mb-3">Report Includes:</h5>
                                ...
                                <button
                                  ...
                                  className={`px-4 py-2 rounded-xl font-medium transition-all text-sm flex items-center gap-2 ${
                                    idx === 0
                                      ...
                                      : 'bg-slate-700/40 hover:bg-slate-700/60 border border-slate-600/50 text-white'
                                  }`}
                                >
            ...
            {(activeSection === 'all' || activeSection === 'compliance') && (
              <div className="mb-8">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    <h3 className="text-lg font-semibold text-white">Regulatory Compliance & Audit Reports</h3>
                    <p className="text-xs text-slate-400">Reports required by federal/state law or regulatory agencies</p>
                  </div>
                </div>
                      ...
                      <div key={report.id} className={`bg-slate-800/40 border ${report.nextDueUrgent ? 'border-amber-500/30' : 'border-slate-700/50'} rounded-xl overflow-hidden`}>
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-sm text-slate-400 mb-2">{report.description}</p>
                            <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500">
                              <span>Authority: <span className="text-slate-300">{report.regulatory}</span></span>
                              <span>Frequency: <span className="text-slate-300">{report.frequency}</span></span>
                            </div>
                          </div>
                          {isExpanded ? <ChevronUp className="w-5 h-5 text-slate-400 mt-1" /> : <ChevronDown className="w-5 h-5 text-slate-400 mt-1" />}
                          ...
                          <div className="px-5 pb-5 space-y-4">
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                              ...
                              <div className={`bg-slate-900/40 border ${report.nextDueUrgent ? 'border-amber-500/30' : 'border-slate-700/30'} rounded-xl p-3`}>
                                ...
                                <p className={`text-sm font-medium ${report.nextDueUrgent ? 'text-amber-400' : 'text-slate-300'}`}>{report.nextDue}</p>
                              </div>
                            </div>
                            ...
                            {report.dataStatus && (
                              <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-3">
                                ...
                                {report.filingDeadline && <p className="text-xs text-slate-400 mt-1">Filing Deadline: {report.filingDeadline}</p>}
                              </div>
                            )}
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400">Completed reports available for download and review</p>
                    </div>
                  </div>
                      ...
                      <div key={report.id} className="bg-slate-800/40 border border-slate-700/50 rounded-xl overflow-hidden">
                          ...
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex items-start gap-4 flex-1">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-slate-400 mb-2">Generated: {report.generated} • By: {report.generatedBy}</p>
                                <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500">
                        ...
                        {isExpanded && (
                          <div className="px-5 pb-5 border-t border-slate-700/30 pt-4 space-y-4">
                            <div className="bg-slate-900/40 border border-slate-700/30 rounded-xl p-4">
                              <h5 className="text-sm font-semibold text-white mb-3">Report Summary</h5>
                              <div className="space-y-1.5">
                            ...
                            <div className="flex flex-wrap gap-3">
                              ...
                              <button className="px-4 py-2 bg-slate-800/40 hover:bg-slate-800/60 border border-slate-700/50 text-slate-300 rounded-xl font-medium transition-all text-sm flex items-center gap-2">
            ...
            <div className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-5">
              <h4 className="text-sm font-semibold text-white mb-3 flex items-center gap-2">
                <Shield className="w-4 h-4 text-slate-400" /> GCSO Report Compliance & Retention Requirements
              </h4>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
                <div className="bg-slate-900/40 border border-slate-700/30 rounded-lg p-3">
                  ...
                  <p className="text-sm text-white font-medium">75 years</p>
                  <p className="text-xs text-slate-400">GA Records Retention</p>
                </div>
                <div className="bg-slate-900/40 border border-slate-700/30 rounded-lg p-3">
                  <p className="text-xs text-slate-500 mb-1">POST Certifications</p>
                  <p className="text-sm text-white font-medium">Permanent</p>
                  <p className="text-xs text-slate-400">O.C.G.A. § 35-8-1</p>
                </div>
                <div className="bg-slate-900/40 border border-slate-700/30 rounded-lg p-3">
                  <p className="text-xs text-slate-500 mb-1">EEO-1 Reports</p>
                  <p className="text-sm text-white font-medium">3 years</p>
                  <p className="text-xs text-slate-400">Title VII, EEOC</p>
                </div>
                <div className="bg-slate-900/40 border border-slate-700/30 rounded-lg p-3">
                  <p className="text-xs text-slate-500 mb-1">FMLA Records</p>
                  <p className="text-sm text-white font-medium">3 years</p>
                  <p className="text-xs text-slate-400">29 CFR Part 825</p>
                </div>
              </div>
      ...
      <div className="fixed bottom-6 right-6 z-50">
        ...
        {supportOpen && (
          <div className="absolute bottom-16 right-0 w-80 bg-slate-900/95 backdrop-blur-xl border border-slate-700/50 rounded-2xl shadow-2xl overflow-hidden">
            <div className="p-4 border-b border-slate-700/50">
              <h3 className="text-sm font-semibold text-white">HR Reports Support</h3>
            ...
            <div className="p-4 space-y-3">
              <div className="p-3 bg-slate-800/40 rounded-lg hover:bg-slate-800/60 cursor-pointer transition-colors">
                <p className="text-sm font-medium text-white">HR Director</p>
                <p className="text-xs text-slate-400">Report authorization, confidential data access</p>
                <p className="text-xs text-amber-400 mt-1">Ext. 4500</p>
              </div>
              <div className="p-3 bg-slate-800/40 rounded-lg hover:bg-slate-800/60 cursor-pointer transition-colors">
                <p className="text-sm font-medium text-white">Report Administrator</p>
                <p className="text-xs text-slate-400">HR Specialist J. Martinez — report generation, data queries</p>
                <p className="text-xs text-amber-400 mt-1">Ext. 4505</p>
              </div>
              <div className="p-3 bg-slate-800/40 rounded-lg hover:bg-slate-800/60 cursor-pointer transition-colors">
                <p className="text-sm font-medium text-white">Training Division</p>
                <p className="text-xs text-slate-400">POST compliance data, certification reports</p>
                <p className="text-xs text-amber-400 mt-1">Ext. 4521</p>
              </div>
              <div className="p-3 bg-slate-800/40 rounded-lg hover:bg-slate-800/60 cursor-pointer transition-colors">
                <p className="text-sm font-medium text-white">County Budget Office</p>
                <p className="text-xs text-slate-400">Staffing reports, budget justification data</p>
              </div>
              <div className="p-3 bg-slate-800/40 rounded-lg hover:bg-slate-800/60 cursor-pointer transition-colors">
