...
const UnitManagement = () => {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'Meal Break': return 'bg-slate-500/20 text-slate-400 border-slate-500/30';
      ...
      <div className="p-4 lg:p-6">
        <div className="mb-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className="text-slate-400">Real-Time Unit Status & Fleet Management</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          <div className="bg-slate-800 border border-slate-700 rounded-lg p-4">
            <div className="text-sm text-slate-400 mb-1">Total Units ({selectedShift === 'all' ? 'All Shifts' : selectedShift})</div>
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-lg p-4">
            <div className="text-sm text-slate-400 mb-1">Out of Service</div>
            <div className="text-2xl font-bold text-slate-400">{stats.outOfService}</div>
          </div>
        </div>
          ...
          {filteredUnits.map((unit) => (
            ...
            >
              ...
              <div className="flex items-center justify-between mb-4">
                <div>
                  <div className="text-xl font-bold text-white mb-1">{unit.callSign}</div>
                  <div className="text-sm text-slate-400">{unit.vehicle}</div>
                </div>
              ...
              <div className="bg-slate-900/50 border border-slate-700 rounded-lg p-3 mb-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <div className="text-sm text-white font-medium">{unit.officer}</div>
                </div>
                <div className="text-xs text-slate-400">Badge: {unit.badge}</div>
                {unit.partner && (
                  <div className="mt-2 pt-2 border-t border-slate-700">
                    <div className="text-sm text-white font-medium">{unit.partner}</div>
                    <div className="text-xs text-slate-400">Badge: {unit.partnerBadge}</div>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-2 text-sm mb-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-white text-xs">{unit.location}</span>
                </div>
                {unit.currentCall && (
                  <div className="flex items-center gap-2">
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                <div className="bg-slate-900/50 border border-slate-700 rounded p-2">
                  <div className="text-slate-400 mb-1">Mileage</div>
                  <div className="text-white font-bold">{unit.mileage.toLocaleString()}</div>
                </div>
                <div className="bg-slate-900/50 border border-slate-700 rounded p-2">
                  <div className="text-slate-400 mb-1">Fuel</div>
