...
export default function BudgetResources() {
  ...
  const [actionedBudgetActions, setActionedBudgetActions] = useState(new Set());
  const budgetToastTimerRef = useRef(null);
  ...
  // Budget Command Actions data
  const budgetCommandActions = [
    {
      id: 'bca-1',
      urgency: 'Critical',
      decision: 'Approve Emergency HVAC Repair — Detention H2-Pod.',
      ifIgnored: 'If ignored, ACA accreditation is at risk in 72 hours — pod temperature currently 84°F, USMS inspection Dec 12.',
    ...
    },
    {
      id: 'bca-2',
      urgency: 'Critical',
      decision: 'Authorize Overtime Spending Override — B-Shift Patrol staffing at 75%.',
      ifIgnored: 'If ignored, Patrol stays below minimum staffing — response time degrades 18% in Zones 4 and 7.',
      timeSensitivity: 'Deadline: Before 18:00 today',
      countdown: 'Today',
      financial: '$8,320',
      actionLabel: 'Approve',
    },
    {
      id: 'bca-3',
      urgency: 'High',
      decision: 'Approve Lateral Hiring Budget — 2 patrol positions to reduce OT overrun.',
      ifIgnored: 'If ignored, overtime spend stays 19% over allocation — year-end overrun projected at $1.2M.',
      timeSensitivity: 'Deadline: Before Q4 ends',
      countdown: '61 days remaining',
      financial: null,
      actionLabel: 'Approve',
    },
    {
      id: 'bca-4',
      urgency: 'Normal',
      decision: 'Review and sign Q4 Reallocation Plan — Training surplus to Patrol.',
      ifIgnored: 'If ignored, Patrol division closes FY in deficit — impacts next-year allocation request.',
      timeSensitivity: 'Deadline: End of week',
      countdown: null,
      financial: '$300,000 reallocation',
      actionLabel: 'Escalate',
    },
  ];

  // Division Budget Risk data
  const divisionBudgetRisks = [
    {
      id: 'dbr-1',
      division: 'Patrol Division',
      percentSpent: 85,
      risk: 'High',
      bottleneck: 'OT 19% over allocation. Trending $150K deficit by year-end without action.',
    ...
    },
    {
      ...
      percentSpent: 85,
      risk: 'High',
      bottleneck: 'Emergency HVAC repair $23.5K unapproved. ACA compliance risk if unresolved.',
      action: 'View Division',
      tabTarget: 'divisions',
    },
    {
      ...
      percentSpent: 86,
      risk: 'Moderate',
      bottleneck: '$300K available committed to active ops. Surveillance extension adds $12K.',
      action: 'View Division',
      tabTarget: 'divisions',
    },
    {
      ...
      percentSpent: 81,
      risk: 'Low',
      bottleneck: '$600K surplus available for reallocation to Patrol and Administration.',
      action: 'View Division',
      tabTarget: 'divisions',
    },
  ];

  const getUrgencyAccentBudget = (urgency) => {
    if (urgency === 'Critical') return 'border-l-red-500';
    if (urgency === 'High') return 'border-l-amber-500';
    return 'border-l-blue-500';
  };

  const getUrgencyBadgeBudget = (urgency) => {
  ...
  const getActionBtnBudget = (urgency) => {
  ...
  const getRiskBarBudget = (risk) => {
    if (risk === 'High') return 'bg-red-500';
    if (risk === 'Moderate') return 'bg-amber-500';
    return 'bg-emerald-500';
  };

  const handleBudgetCommandAction = (action) => {
    setActionedBudgetActions(prev => new Set([...prev, action.id]));
    const msg = `${action.actionLabel} recorded — ${action.decision.slice(0, 55)}…`;
    setBudgetToast({ message: msg, type: 'success' });
    if (budgetToastTimerRef.current) clearTimeout(budgetToastTimerRef.current);
    budgetToastTimerRef.current = setTimeout(() => setBudgetToast(null), 3500);
  };

  const criticalBudgetCount = budgetCommandActions.filter(a => a.urgency === 'Critical').length;
  const pendingBudgetCount = budgetCommandActions.filter(a => !actionedBudgetActions.has(a.id)).length;
