...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex">
      {/* Sidebar */}
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 border-r border-slate-800/30 bg-slate-950 flex flex-col transform transition-all lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} ${sidebarCollapsed ? 'w-20' : 'w-64'}`}>
        <div className="p-4 border-b border-slate-800/30 flex items-center justify-between">
          ...
          <button
            ...
            className="p-2 hover:bg-slate-900/50 rounded-lg transition-colors hidden lg:block"
          >
            {sidebarCollapsed ? <ChevronRight className="w-5 h-5 text-slate-500" /> : <ChevronLeft className="w-5 h-5 text-slate-500" />}
          </button>
        </div>
              ...
              <div key={item.id}>
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-all ${
                    isActive || isStaffingActive ? 'bg-amber-500 text-white' : 'text-slate-500 hover:bg-slate-900/50 hover:text-slate-300'
                ...
                >
                  ...
                  {!sidebarCollapsed && (
                    <>
                      ...
                      {item.badge && <span className={`px-2 py-0.5 rounded-full text-xs ${isActive ? 'bg-white/20' : 'bg-red-500/80 text-white'}`}>{item.badge}</span>}
                      {item.hasSubmenu && (
                      ...
                      return (
                        <button
                          ...
                          className={`w-full flex items-center gap-3 px-4 py-2 rounded-lg transition-all text-sm ${
                            isSubActive ? 'bg-amber-500/20 text-amber-400' : 'text-slate-600 hover:bg-slate-900/30 hover:text-slate-400'
                          }`}
                        >
        ...
        <div className="border-t border-slate-800/30">
          {!sidebarCollapsed && (
            <div className="px-4 py-3">
              <p className="text-[11px] text-slate-700 text-center">Gwinnett County Sheriff's Office</p>
            </div>
          )}
          ...
          <div className="p-4">
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-all text-slate-500 hover:bg-slate-900/40 hover:text-slate-400 ${sidebarCollapsed ? 'justify-center' : ''}`}
      ...
      {logoutConfirmOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          />
          <div className="relative bg-slate-900 border border-slate-800/30 rounded-xl p-6 max-w-sm w-full">
            <div className="flex items-center gap-4 mb-4">
              <div className="w-10 h-10 bg-slate-800/40 rounded-lg flex items-center justify-center">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Top Command Bar */}
        <header className="border-b border-slate-800/30 bg-slate-950 sticky top-0 z-30">
          <div className="px-4 lg:px-6 py-3 flex items-center justify-between gap-4">
            {/* Mobile menu button and Search */}
            <div className="flex items-center gap-4 flex-1 min-w-0">
              <button
                onClick={() => setSidebarOpen(!sidebarOpen)}
                className="lg:hidden p-2 hover:bg-slate-900/50 rounded-lg"
              >
              ...
              <div className="flex-1 max-w-xl relative hidden sm:block">
                ...
                <input
                  type="text"
                  placeholder="Search..."
                  className="w-full pl-10 pr-4 py-2 bg-slate-900/30 border border-slate-800/30 rounded-lg text-white placeholder-slate-600 focus:outline-none focus:border-slate-700 text-sm"
                />
              </div>
            </div>

            {/* System Health + Notifications + Profile */}
            <div className="flex items-center gap-3 lg:gap-4">
              ...
              <div className="h-6 w-px bg-slate-800/30 hidden lg:block"></div>

              {/* Notifications */}
              <div className="relative">
                <button
                  onClick={() => setNotificationsOpen(!notificationsOpen)}
                  className="notifications-trigger p-2 hover:bg-slate-900/50 rounded-lg relative"
                >
                ...
                {notificationsOpen && (
                  <div className="notifications-dropdown absolute right-0 top-full mt-2 w-80 bg-slate-900 border border-slate-800/30 rounded-lg z-50">
                    <div className="p-3 border-b border-slate-800/30">
                      <div className="flex items-center justify-between">
                        <h3 className="text-sm font-medium text-white">Notifications</h3>
                        <span className="px-1.5 py-0.5 bg-red-500/10 border border-red-500/20 text-red-400 text-[11px] rounded">
                    ...
                    <div className="max-h-80 overflow-y-auto">
                      {notifications.map(notification => (
                        <div
                          key={notification.id}
                          className="p-3 border-b border-slate-800/20 hover:bg-slate-800/20 cursor-pointer transition-colors"
                        >
                          <div className="flex items-start gap-2.5">
                            <div className={`w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0 ${
                              notification.urgent ? 'bg-amber-500' : 'bg-slate-600'
                            }`}></div>
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-medium text-white mb-0.5">{notification.title}</p>
                              <p className="text-[11px] text-slate-500 mb-1">{notification.message}</p>
                              <p className="text-[11px] text-slate-700">{notification.time}</p>
                            </div>
                          </div>
                        </div>
                      ))}
                    </div>
                    <div className="p-2.5 border-t border-slate-800/30">
                      <button className="w-full text-center text-xs text-slate-500 hover:text-slate-400 font-medium">
              ...
              <div className="h-6 w-px bg-slate-800/30 hidden sm:block"></div>

              {/* Profile Menu */}
              <div className="relative">
                <button
                  onClick={() => setProfileMenuOpen(!profileMenuOpen)}
                  className="profile-trigger flex items-center gap-2 hover:bg-slate-900/50 rounded-lg p-1.5 transition-colors"
                >
                  ...
                  <div className="hidden sm:block text-left">
                    ...
                    <p className="text-[11px] text-slate-600">Administrator</p>
                  </div>
                  <ChevronDown className="w-4 h-4 text-slate-600 hidden sm:block" />
                </button>

                {profileMenuOpen && (
                  <div className="profile-dropdown absolute right-0 top-full mt-2 w-52 bg-slate-900 border border-slate-800/30 rounded-lg z-50">
                    <div className="p-3 border-b border-slate-800/30">
                      <p className="text-sm font-medium text-white">Sheriff Thompson</p>
                      <p className="text-[11px] text-slate-600">sheriff.thompson@gwinnettcounty.com</p>
                      <span className="inline-block mt-1.5 px-1.5 py-0.5 bg-amber-500/10 border border-amber-500/20 text-amber-400 text-[11px] rounded">
                        Administrator
                      </span>
                    </div>
                    <div className="p-1.5">
                      <button
                        ...
                        className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-slate-400 hover:bg-slate-800/30 hover:text-white transition-colors text-sm"
                      >
                    ...
                    <div className="p-1.5 border-t border-slate-800/30">
                      <button
                        onClick={() => {
                          setProfileMenuOpen(false);
                          setLogoutConfirmOpen(true);
                        }}
                        className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-slate-500 hover:bg-slate-800/30 hover:text-red-400 transition-colors text-sm"
                      >
